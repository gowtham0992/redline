...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_budget_arguments(benchmark_parser)
    benchmark_parser.set_defaults(func=cmd_benchmark, command_name="benchmark")

    budget_parser = subparsers.add_parser("budget", help="estimate suite eval runtime and CI scale without replaying")
    _add_budget_arguments(budget_parser)
    budget_parser.set_defaults(func=cmd_benchmark, command_name="budget")
...
def _add_budget_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("suite", nargs="?", help="suite JSON generated by redline suite")
    parser.add_argument("--config", default=DEFAULT_CONFIG_PATH, help="config path to read")
    parser.add_argument("--timeout", type=float, help="per-case timeout in seconds")
    parser.add_argument("--workers", type=int, help="number of replay workers")
    parser.add_argument("--max-seconds", type=float, help="exit 1 when worst-case eval budget exceeds this")
    parser.add_argument("--out-json", help="write benchmark report JSON")
    parser.add_argument("--out-md", help="write benchmark report Markdown")
    parser.add_argument("--github-summary", action="store_true", help="append benchmark to GITHUB_STEP_SUMMARY")
    parser.add_argument(
        "--measure-local",
        action="store_true",
...
def cmd_benchmark(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        print(format_benchmark_report(report, command_name=getattr(args, "command_name", "benchmark")), end="")
    if args.out_json:
        write_json(args.out_json, report)
    if args.out_md:
        write_text(args.out_md, format_benchmark_markdown(report, command_name=getattr(args, "command_name", "benchmark")))
    if args.github_summary:
        _append_github_step_summary(format_benchmark_markdown(report, command_name=getattr(args, "command_name", "benchmark")))
